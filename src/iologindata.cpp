...
uint64_t IOLoginData::createAccount(std::string name, std::string password, std::string email)
{
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
		...
		for(ItemMap::reverse_iterator rit = itemMap.rbegin(); rit != itemMap.rend(); ++rit)
		{
			...
			else
			{
				...
				if(it != itemMap.end())
				{
					...
					{
						//if(container->getItem() && container->getItem()->getID() == 12332)
						//	std::cout << "**LOAD BAG ITEM SYSTEM** " << item->getID() << std::endl;
	...
	//load catch list set
	query.str("");
	query << "SELECT * FROM `player_catch` WHERE `player_id` = " << player->getGUID();

	if((result = db->storeQuery(query.str())))
	{
    	do{
           std::string pokeName = result->getDataString("poke");
    	   
   	       bool dex = (bool)result->getDataInt("dex");
           bool use = (bool)result->getDataInt("use");
   	       bool Catch = (bool)result->getDataInt("catch");
   	       uint16_t PBall = result->getDataInt("pokeball");
           uint16_t GBall = result->getDataInt("greatball");
   	       uint16_t SBall = result->getDataInt("superball");
   	       uint16_t UBall = result->getDataInt("ultraball");
   	       uint16_t SaBall = result->getDataInt("saffariball");
   	       uint16_t MBall = result->getDataInt("masterball");
   	       uint16_t PRBall = result->getDataInt("premierball");
		   uint16_t VBall = result->getDataInt("vball");
   	       
   	       player->setPlayerCatch(pokeName, dex, use, Catch);
           player->setPlayerCatchCount(pokeName, POKE, PBall);
           player->setPlayerCatchCount(pokeName, GREAT, GBall);
           player->setPlayerCatchCount(pokeName, SUPER, SBall);
           player->setPlayerCatchCount(pokeName, ULTRA, UBall);
           player->setPlayerCatchCount(pokeName, SAFFARI, SaBall);
           player->setPlayerCatchCount(pokeName, MASTER, MBall);
           player->setPlayerCatchCount(pokeName, PREMIER, PRBall);
		   player->setPlayerCatchCount(pokeName, VBALL, VBall);
        }
		while(result->next());
		result->free();
	}
...
void IOLoginData::loadItems(ItemMap& itemMap, DBResult* result)
{
	do
	{
		...
		if(Item* item = Item::CreateItem(result->getDataInt("itemtype"), result->getDataInt("count")))
		{
			...
			std::cout << "**LOAD ITEM SYSTEM** " << result->getDataInt("itemtype") << std::endl;
	}
	while(result->next());
}
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	{*/
        ...
        //icon system	
	    /*for (ItemBlockList::iterator its = itemList.begin(); its != itemList.end(); ++its){
            upIcon(player->getGUID(), 2222, 11066);
			//g_game.transformItem(its->first, 11068, 1);
        }*/
	//}
	...
	//save market
	IOMarket::getMarket()->savePlayerItemsMarket(player);
    ...
    for (it = player->autoLootList.begin(); it != player->autoLootList.end(); ++it, ++i)
    	propWriteStreamAutoLoot.ADD_USHORT(*it);
 
    uint32_t lootlistSize = 0;
	//size_t lootlistSize;
	const char* autolootlist = propWriteStreamAutoLoot.getStream(lootlistSize);
 
	query.str("");
	DBInsert autolootQuery(db);
	autolootQuery.setQuery("INSERT INTO `player_autoloot` (`player_id`, `autoloot_list`) VALUES ");
    ...
    //CatchSave(player);
    query.str("");
	query << "DELETE FROM `player_catch` WHERE `player_id` = " << player->getGUID();
	if(!db->executeQuery(query.str()))
		return false;
		
	query_insert.setQuery("INSERT INTO `player_catch` (`player_id`, `poke`, `dex`, `use`, `catch`, `pokeball`, `greatball`, `superball`, `ultraball`, `saffariball`, `masterball`, `premierball`, `vball`) VALUES ");
	
   	for(PlayerCatch::iterator itC = player->playerCatch.begin(); itC != player->playerCatch.end(); ++itC)
   	{
		sprintf(buffer, "%u, %s, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d", player->getGUID(), db->escapeString(itC->second.poke).c_str(), itC->second.dex, itC->second.use, itC->second.catchP, itC->second.pokeball,itC->second.greatball,itC->second.superball,itC->second.ultraball,itC->second.saffariball,itC->second.masterball,itC->second.premierball, itC->second.vball);
		if(!query_insert.addRow(buffer))
			return false;
    }
    if (!query_insert.execute())
...
bool IOLoginData::saveItemsViktor(const Player* player, const ItemBlockList& itemList, std::string tabela)
{
	Database* db = Database::getInstance();  
	...
	for(ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it, ++runningId)
	{
		...
		//attributes stream
		PropWriteStream itemInfoStream;
		item->serializeAttr(itemInfoStream);

		uint32_t attributesSize = 0;
		const char* attributes = itemInfoStream.getStream(attributesSize);
		...
		if(!db->executeQuery(query.str()))
		{
			std::cout << "**SAVE ITEM SAVESITEMS** NAO EXECUTOU ITEM PLAYER " << player->getGUID() << " ITEMID " << item->getID() << std::endl;	
			continue;
		}
	...
	while(!stackList.empty())
	{
		...
		for(ItemList::const_iterator items = container->getItems(); items != container->getEnd(); ++items, ++runningId)
		{
			Item* item = *items;
			if(!item)
			{
				std::cout << "**SAVE ITEM SYSTEM DB** NAO ER ITEM PLAYER " << player->getGUID() << " ITEMID " << std::endl;	
				continue;
			} 
			...
			if(!db->executeQuery(query.str()))
			{
				std::cout << "**SAVE ITEM SAVESITEMS** NAO EXECUTOU ITEM CONTAINER PLAYER " << player->getGUID() << " ITEMID " << item->getID() << std::endl;				
				continue;
			}
		}
	}
...
bool IOLoginData::playerNewMail(Creature* actor, std::string name, uint32_t townId, Item* item)
{
	Player* player = g_game.getPlayerByNameEx(name);
	if(!player)
		return false;
	...
	g_game.transformItem(item, item->getID());
	bool result = true, opened = player->getContainerID(depot) != -1;
...
bool IOLoginData::playerPokeCP(Creature* actor, std::string name, uint32_t townId, Item* item)
{
	Player* player = g_game.getPlayerByNameEx(name);
	if(!player)
		return false;
...
bool IOLoginData::createCharacter(uint32_t accountId, std::string characterName, int32_t pokemonId, uint16_t sex, uint8_t townId, uint8_t worldId)
{
	if(playerExists(characterName))
		return false;

	Vocation* pokemon = Vocations::getInstance()->getVocation(pokemonId); 
	Vocation* rookVoc = Vocations::getInstance()->getVocation(0);

	uint16_t healthMax = 150, manaMax = 0, lookType = 136;
    double capMax = 400.00;
	if(sex % 2)
		lookType = 128;
	...
	if(tmpLevel > 0)
	{
		...
		{
			tmpLevel = level - 8;
			healthMax += pokemon->getGain(GAIN_HEALTH) * tmpLevel;
			manaMax += pokemon->getGain(GAIN_MANA) * tmpLevel;
			capMax += pokemon->getGainCap() * tmpLevel;
		}
	}

	Database* db = Database::getInstance();
	DBQuery query;
//g_config.getNumber(ConfigManager::SPAWNTOWN_ID)
    char town[10];
    sprintf(town, "%d", townId);
	char world[10];
    sprintf(world, "%d", worldId);
	query << "INSERT INTO `players` (`id`, `name`, `world_id`, `group_id`, `account_id`, `level`, `vocation`, `health`, `healthmax`, `experience`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `maglevel`, `mana`, `manamax`, `manaspent`, `soul`, `town_id`, `posx`, `posy`, `posz`, `conditions`, `cap`, `sex`, `lastlogin`, `lastip`, `skull`, `skulltime`, `save`, `rank_id`, `guildnick`, `lastlogout`, `blessings`, `online`) VALUES (NULL, " << db->escapeString(characterName) << ", " << world << ", 1, " << accountId << ", " << level << ", " << pokemonId << ", " << healthMax << ", " << healthMax << ", " << exp << ", 68, 76, 78, 39, " << lookType << ", 0, " << g_config.getNumber(ConfigManager::START_MAGICLEVEL) << ", " << manaMax << ", " << manaMax << ", 0, 100, " << town << ", " << g_config.getNumber(ConfigManager::SPAWNPOS_X) << ", " << g_config.getNumber(ConfigManager::SPAWNPOS_Y) << ", " << g_config.getNumber(ConfigManager::SPAWNPOS_Z) << ", 0, " << capMax << ", " << sex << ", 0, 0, 0, 0, 1, 0, '', 0, 0, 0)";
	return db->executeQuery(query.str());
}
...
bool IOLoginData::upIcon(uint32_t guid, uint32_t value, uint32_t valu)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "UPDATE `player_depotitems` SET `itemtype` = " << value << " WHERE `player_id` = " << guid << " AND `itemtype` = " << valu << db->getUpdateLimiter();
	return db->executeQuery(query.str());
}


bool IOLoginData::deleteMarket(Player* player, uint16_t id)
{
    Database* db = Database::getInstance();
	DBQuery query;
	...
	if((result = db->storeQuery(query.str())))
	{
        if (player->getGUID() != result->getDataInt("player_id"))
        {                
            result->free();
        ...
        if(buyItemMarket(player, result, result->getDataInt("count"), id, true))
        {
            query.str("");
   	        query << "DELETE FROM `market_items` WHERE `id` = " << id << ";";
	    	db->executeQuery(query.str());
	    	player->sendExtendedOpcode(155, "venda cancelada.");
	    	result->free();
	    	return true;
        }
    }
...
uint64_t IOLoginData::getMoneyMarket(Player* player, bool type)
{
...
bool IOLoginData::removeMoneyMarket(Player* player, bool type, uint64_t price)
{
	bool bol = false;
	if(type)
...
bool IOLoginData::addMoneyMarket(std::string player, bool type, uint64_t value)
{ 
	if(type)
	{
        ...
        playerPokeCP(NULL, player, 1, item); 
        value -= valueC;
        while(value > 0) 
        {
            ...
            playerPokeCP(NULL, player, 1, itemW);
            value -= valueC;
        }
	} 
	else
...
bool IOLoginData::buyMarket(Player* player, uint16_t id, uint16_t count)
{
    Database* db = Database::getInstance();
	DBQuery query;
	//query << "SELECT `player_id`, `cost`, `itemtype`, `count`, `attributes`, `time`, `money` FROM `market_items` WHERE `id` = " << id << ";";
	query << "SELECT * FROM `market_items` WHERE `id` = " << id << ";";
	DBResult* result;
	if((result = db->storeQuery(query.str())))
	{
        ...
        if(time(NULL) >= result->getDataInt("time"))
        {
        ...
        bool moneyB = (bool)result->getDataInt("money");
        uint64_t price = (uint64_t)result->getDataInt("cost");//(result->getDataInt("cost")*count)*100;
        if(!(bool)result->getDataInt("stack"))
           price = (uint64_t)(price*count);
           
        uint64_t money = (uint64_t)getMoneyMarket(player, moneyB);
        if(money < price)
        {
		...
		if(money >= price && buyItemMarket(player, result, count, id))
		{
			std::string vendedor;
			uint32_t vendedorId = result->getDataInt("player_id");
			getNameByGuid(vendedorId, vendedor, false);
            //g_game.removeMoney(player, price);
            removeMoneyMarket(player, moneyB, (uint64_t)price);
			//g_game.addMoneyMarket(vendedor, price);
			addMoneyMarket(vendedor, moneyB, (uint64_t)price); 
			player->sendExtendedOpcode(155, "Compra concluida");
			result->free();
	    	return true;
        }
...
bool IOLoginData::buyItemMarket(Player* player, DBResult* result, uint16_t count, uint16_t id, bool cancel)
{
	uint64_t attrSize = 0;
	const char* attr = result->getDataStream("attributes", attrSize);
	...
	if(Item* item = Item::CreateItem(result->getDataInt("itemtype"), count))
	{
		if(!item->unserializeAttr(propStream))
		{
			std::cout << "**MARKET SYSTEM** [Warning - IOLoginData::loadItems] Unserialize error for item with id " << item->getID() << std::endl;
			delete item;
			return false;
		}
		
		uint16_t quant = result->getDataInt("count");
		if(count <= quant)
        {
           /* --viktor--- uint16_t value = std::min((uint16_t)100, (uint16_t)count);  
            item->setItemCount(value);
            playerMail(NULL, player->getName(), 1, item); 
            //if(count >= 100)
            count -= value;
            while(count > 0)
            {
                 uint16_t value = std::min((uint16_t)100, (uint16_t)count);
                 Item* itemW = Item::CreateItem(item->getID()-1, value);
                 itemW->setItemCount(value);
                 playerMail(NULL, player->getName(), 1, itemW);
                 count -= value;
            }*/
			//item->setItemCount(count);
			//Thing* thing = player->__getThing(3);
			//playerMail(NULL, player->getName(), 1, item);
			Database* db = Database::getInstance();
			DBQuery query;
            query.str("");
			
			if(cancel || count == quant)
				query << "DELETE FROM `market_items` WHERE `id` = " << id << ";";
			else
	    		query << "UPDATE `market_items` SET `count` = " << quant-count << " WHERE `id` = " << id;
			
	    	db->executeQuery(query.str());
	    	
	    	uint16_t value = std::min((uint16_t)100, (uint16_t)count);  
            item->setItemCount(value);
            playerNewMail(NULL, player->getName(), 1, item); 
            //if(count >= 100)
            count -= value;
            while(count > 0)
            {
                 uint16_t value = std::min((uint16_t)100, (uint16_t)count);
                 Item* itemW = Item::CreateItem(item->getID(), value);
                 itemW->setItemCount(value);
                 playerNewMail(NULL, player->getName(), 1, itemW);
                 count -= value;
            }
	    	//result->free();
	    	//player->sendExtendedOpcode(155, "Compra concluida");
	...
	//	std::cout << propStream;
		//g_game.internalAddItem(NULL, thing->getItem()->getContainer(), item);
		//g_game.transformItem(item, item->getID(), count);
		return false;
	}
...
bool IOLoginData::sellMarket(const Player* player, const Item* item, uint16_t count, uint64_t preco, bool money, bool stack)
{
    /*Database* db = Database::getInstance();
    DBInsert query_insert(db);*/
    if(preco < 1)
    ...
    query << "INSERT INTO `market_items` (`player_id`, `cost`, `itemtype`, `count`, `attributes`, `time`, `money`, `stack`) ";
	query << "VALUES (" << player->getGUID() << ", " << preco << ", " << item->getID() << ", " << buffer;
	query << ", " << db->escapeBlob(attributes, attributesSize) << ", " << time(NULL)+(2*24*60*60) << ", " << money << ", " << stack <<  ")";
	
	
	//std::cout << buffer << " >> " << time(NULL);
    //query_insert.setQuery("INSERT INTO `market_items` (`player_id`, `cost`, `itemtype`, `count`, `attributes`, `time`) VALUES ");
	//if(!sellItemMarket(player, item, count, preco, query_insert))
	//	return false;
