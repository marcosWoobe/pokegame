	...
	Creature()
{
    ...
    passive = mType->isPassive;
    price = mType->isPrice;
    ride = mType->isRide;
    fly = mType->isFly;
    surf = mType->isSurf;
    rocksmash = mType->isRockSmash;
    dig = mType->isDig;
    cut = mType->isCut;
    blink = mType->isBlink;
    light = mType->isLight;
    cmid = mType->isCMid;
    headbutt = mType->isHeadButt;
...
void Monster::doRegisterDamageMap(Creature* player, int32_t damage){
    ...
    if (damageMap[name]){ // ja esta registrado
        damageMap[name] += damage;
    }else{//nao esta registrado
        damageMap[name] = damage;
    }
}
void Monster::clearBlackList(){
    listBlack.erase(listBlack.begin(), listBlack.end());
}
...
void Monster::addToBlackList(Creature* creature){
    std::string name = creature->getName();
    if (creature->isSummon())
        name = creature->getMaster()->getName();

    listBlack[name] = true;
    if (!isSummon() && !getAttackedCreature())
        selectTarget(creature);
}
void Monster::removeFromBlackList(Creature* creature){
    std::string name = creature->getName();
    listBlack[name] = false;
}
bool Monster::isInBlackList(const Creature* creature){
    std::string name = creature->getName();
    if (listBlack[name])
        return true;
    if (creature->isSummon() && listBlack[creature->getMaster()->getName()])
        return true;
	...
	const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	else
	{
        ...
        if (isSummon() && !isInMasterRange()){//poketele
            doTeleportToMaster();
        }   
...
bool Monster::isFriend(const Creature* creature)
{
	...
	return tmpPlayer && ((tmpPlayer == getMaster() || masterPlayer->isPartner(tmpPlayer)) || !isInBlackList(tmpPlayer));
}
...
bool Monster::isTarget(Creature* creature)
{
    ...
    // {
    //     if(creature->getPlayer() || creature->isSummon())
    //         return false;
    // }
...
#endif
if (creature->getPlayer() && creature->getSummonCount() > 0){
        return false;
    }
    if (passive && !isInBlackList(creature))
        return false;
	...
	if(isSummon())
	{
		if(!getAttackedCreature())
		{
			...
			if(getMaster() && getMaster()->getAttackedCreature()) //This happens if the monster is summoned during combat
			{
    			...
    			if(getMaster())
				{
                    Position posdrt = getMaster()->getCreature()->getOrderPos();
                    getMaster()->getCreature()->setStatValue("ordered", 0);
                    getMaster()->getCreature()->setOrderPos(posdrt - posdrt);
                }
            }
			else if(getMaster() != followCreature && getMaster()->getCreature()->getStatValue("ordered") != 1) //Our master has not ordered us to attack anything, lets follow him around instead.
	     		setFollowCreature(getMaster());
		}
...
{
	if(!attackedCreature || (isSummon() && attackedCreature == this))// || isSummon())
		return;
	...
	for(SpellList::iterator it = mType->spellAttackList.begin(); it != mType->spellAttackList.end(); ++it)
	{
		...
		if(canUseSpell(myPos, targetPos, *it, interval, inRange))
		{
			if(it->chance >= (uint32_t)random_range(1, 100))
			{
				...
				InstantSpell* spellMove = g_spells->getInstantSpellByName(it->isName.c_str());
				if(it->isName.c_str() == "melee" || it->isName.c_str() == "physical" || it->isName == "melee" || it->isName == "physical")
					it->spell->castSpell(this, attackedCreature);
				else if(spellMove && it->automatic)
					spellMove->castSpell(getCreature(), attackedCreature->getCreature(), it->isName.c_str());
				else if(spellMove && (isGuardian() || !isSummon()))
                    spellMove->castSpell(getCreature(), attackedCreature->getCreature(), it->isName.c_str());
				else if(spellMove && isSummon() && getMaster()->getNpc())
					spellMove->castSpell(getCreature(), attackedCreature->getCreature(), it->isName.c_str());
...
//passive
	...
	if(targetChangeCooldown > 0)
	{
		...
		{
			targetChangeCooldown = 0;
			targetChangeTicks = (uint32_t)100;//mType->changeTargetSpeed;
		}
		else
	...
	if(targetChangeTicks < (uint32_t)100)//mType->changeTargetSpeed)
		return;

	targetChangeTicks = 0;
	targetChangeCooldown = (uint32_t)100;///(uint32_t)mType->changeTargetSpeed;
...
void Monster::onThinkDefense(uint32_t interval)
{
	...
	for(SpellList::iterator it = mType->spellDefenseList.begin(); it != mType->spellDefenseList.end(); ++it)
	{
		...
		if((it->chance >= (uint32_t)random_range(1, 100)))
		{
			...
			InstantSpell* spellMove = g_spells->getInstantSpellByName(it->isName.c_str());
			if(spellMove)
                spellMove->castSpell(this, this, it->isName.c_str());
		}
	}
...
void Monster::pushCreatures(Tile* tile)
{
	...
	// if(effect)
		// g_game.addMagicEffect(tile->getPosition(), MAGIC_EFFECT_BLOCKHIT); // desativado masterx
}
