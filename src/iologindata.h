...
class IOLoginData
{
	public:
		...
		uint64_t createAccount(std::string name, std::string password, std::string email = "");
		...
		bool playerNewMail(Creature* actor, std::string name, uint32_t townId, Item* item);
		bool playerPokeCP(Creature* actor, std::string name, uint32_t townId, Item* item);
		...
		bool createCharacter(uint32_t accountId, std::string characterName, int32_t vocationId, uint16_t sex, uint8_t townId = 1, uint8_t worldId = 1);
		...
		bool upIcon(uint32_t guid, uint32_t value, uint32_t valu);
		
		bool sellMarket(const Player* player, const Item* item, uint16_t count, uint64_t preco, bool money, bool stack);
		bool buyMarket(Player* player, uint16_t id, uint16_t count);
		bool buyItemMarket(Player* player, DBResult* result, uint16_t count, uint16_t id, bool cancel = false);
		bool deleteMarket(Player* player, uint16_t id);
	...
	protected: 
		...
		bool saveItemsViktor(const Player* player, const ItemBlockList& itemList, std::string tabela);
