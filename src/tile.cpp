	...
	uint32_t flags) const
{
	...
	if(ground)
	{
	 if(const Monster* monster = creature->getMonster())
		...
		if(const Monster* monster = creature->getMonster())
		{          
			// Water problem: Monster walking into water
			if(ground->getID() == 11756 || (ground->getID() >= 4821 && ground->getID() <= 4825))
				return RET_NOTPOSSIBLE;
			
			if(ground->getID() >= 4608 && ground->getID() <= 4613)
				return RET_NOTPOSSIBLE;
			
			if (ground->getID() == 4608)
				return RET_NOTPOSSIBLE;
	
			if (ground->getID() == 4613)
				return RET_NOTPOSSIBLE;
	
			if (ground->getID() == 4610)
				return RET_NOTPOSSIBLE;
	
			if (ground->getID() == 4611)
				return RET_NOTPOSSIBLE;
	
			if (ground->getID() == 4612)
				return RET_NOTPOSSIBLE;
	
			if (ground->getID() == 4609)
				return RET_NOTPOSSIBLE;
...
void Tile::__removeThing(Thing* thing, uint32_t count)
{
	...
	else
	{
		for(ItemVector::iterator it = items->getBeginDownItem(); it != items->getEndDownItem(); ++it)
		{
			...
			{
				uint16_t newCount = (uint16_t)std::max((int32_t)0, (int32_t)(item->getItemCount() - count));//65kitem
				updateTileFlags(item, true);
