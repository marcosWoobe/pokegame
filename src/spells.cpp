...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	if(readXMLString(p, "name", strValue))
	{
		...
		{
			"me_le_e", "physical", "poison", "earth", "fire", "ice", "freeze", "energy", "drown", "death", "curse", "holy",
			"lifedrain", "manadrain", "healing", "speed", "outfit", "in_visibles", "drunk", "firefield", "poisonfield",
...
bool InstantSpell::castSpell(Creature* creature, std::string moveS)
{
	if(!BaseSpell::castSpell(creature))
		return false;
	...
	if(casterTargetOrDirection)
	{
		...
		if(target && target->getHealth() > 0)
		{
			...
			var.type = VARIANT_NUMBER;
			var.number = target->getID();
			return internalCastSpell(creature, moveS);
		}
...
bool InstantSpell::castSpell(Creature* creature, Creature* target, std::string moveS)
{
	if(!BaseSpell::castSpell(creature, target))
		return false;

	if(!needTarget)
		return castSpell(creature, moveS);
...
bool InstantSpell::internalCastSpell(Creature* creature, std::string var)
{
	if(isScripted())
		return executeCastSpell(creature, var);
...
bool InstantSpell::executeCastSpell(Creature* creature, std::string var)
{
	//onCastSpell(cid, var)
	if(m_interface->reserveEnv())
	{
		...
		if(m_scripted == EVENT_SCRIPT_BUFFER)
		{
			...
			scriptstream << "local cid = " << env->addThing(creature) << std::endl;
			scriptstream << "local var = " << getName().c_str() << std::endl;
		...
		else
		{
			...
			m_interface->pushFunction(m_scriptId);
			lua_pushnumber(L, env->addThing(creature));
			lua_pushlstring(L, getName().c_str(), getName().length());
...
bool InstantSpell::SummonMonster(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	...
	ReturnValue ret = g_game.placeSummon(creature, param, param);
	if(ret == RET_NOERROR)
	{
...
bool ConjureSpell::playerCastInstant(Player* player, const std::string& param)
{
	...
	if(isScripted())
	{
		...
		var.type = VARIANT_STRING;
		var.text = param;
		return executeCastSpell(player, "");
	}
