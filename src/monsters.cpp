...
{
	canPushItems = canPushCreatures = isSummonable = isIllusionable = isConvinceable = isLureable = isWalkable = hideName = hideHealth = isShiny = false;
    ...
    isPassive = false;
    isPrice = 0;
    isRide = isFly = isSurf = 0;
    isRockSmash = isDig = isCut = isBlink = isLight = isCMid = isHeadButt = false;
	race = RACE_BLOOD;
	skull = SKULL_NONE;
...
// 	std::stringstream lootMsg;
// 	lootMsg << "";
// 	std::stringstream aulootMsg;
// 	aulootMsg << "";
// 	std::stringstream luckyMsg;
// 	luckyMsg << "";
// 	bool begin = true;
// 	bool beginL = true;


// 	if(random_range(0, 30000) <= 10)
// 	{
// 		tmpItem = Item::CreateItem(12339, 1);
// 		corpse->__internalAddThing(tmpItem);
// 		lootMsg << (tmpItem)->getNameDescription();
// 		tmpItem = NULL;
// 		begin = false;
// 	}

// 	if(random_range(0, 100000) <= 10)
// 	{
// 		tmpItem = Item::CreateItem(12618, 1);
// 		corpse->__internalAddThing(tmpItem);
// 		lootMsg << (tmpItem)->getNameDescription();
// 		tmpItem = NULL;
// 		begin = false;
...
// 			if(owner->getAutoLootItem(tmpItem->getID()))
// 			{
// 				aulootMsg << " " << tmpItem->getItemCount() << " " << tmpItem->getName();
// 				Thing* bagT = owner->__getThing(3);
// 				Item* bag = bagT->getItem();
// 				g_game.doPlayerAddItemStacking(owner, bag->getContainer(), tmpItem->getID(), tmpItem->getItemCount());
// 			}else{
...
// 			}
// 		}else{
// 		    if(owner->getSummonCount() < 1)
// 			    continue;

// 			const std::list<Creature*>& summons = owner->getSummons();
// 	        CreatureList::const_iterator pokeLucky = summons.begin();
// 			if(!*pokeLucky)
// 				continue;

// 			Creature* pokeL = (*pokeLucky);
// 			if(!pokeL)
// 				continue;

//             std::string luckySto;
// 			float lucky = 0;
// 			if(pokeL->getStorage(75000, luckySto) && atoi(luckySto.c_str()) >= 10)
//                 lucky = (lucky + 100)/100;
// 			else
// 				continue;


// 			if((tmpItem = createLoot(*it, lucky)))
// 			{
// 				if(!beginL)
// 					luckyMsg << ", ";
// 				else
...
// 				luckyMsg << (tmpItem)->getNameDescription();
// 				if(owner->getAutoLootItem(tmpItem->getID()) && owner->__getThing(3))
// 				{
// 					aulootMsg << " " << tmpItem->getItemCount() << " " << tmpItem->getName();
// 					Thing* bagT = owner->__getThing(3);
// 					Item* bag = bagT->getItem();
...
// 	if(aulootMsg.str() != "")
// 		ss << "Loot do " << nameDescription << ": " << lootMsg.str() << ". " << luckyMsg.str() << aulootMsg.str() << " was auto looted.";
// 	else
// 	    ss << "Loot do " << nameDescription << ": " << lootMsg.str() << ". " << luckyMsg.str();
...
// Item* MonsterType::createLoot(const LootBlock& lootBlock, float lucky)
// {
...
//     uint32_t chanceDrop = lootBlock.chance;
// 	if(lucky > 0)
// 	    chanceDrop *= lucky;
...
// 			//tmpItem = Item::CreateItem(item, random_range(1, lootBlock.count));
// 			tmpItem = Item::CreateItem(item, (random % lootBlock.count + 1));
// 	}
// 	else if(random < chanceDrop)
...
bool Monsters::deserializeSpell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	...
	sb.combatSpell = sb.isMelee = sb.automatic = false;
	...
	if(readXMLString(node, "automatic", strValue))
	{
		sb.automatic = booleanString(strValue);
	}
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	if(readXMLString(root, "race", strValue))
	{
		...
		else if(tmpStrValue == "dark" || atoi(strValue.c_str()) == 22)
			...
			while(tmpNode)
			{
				if(xmlStrcmp(tmpNode->name, (const xmlChar*)"flag") == 0)
				{
					...
					if(readXMLInteger(tmpNode, "price", intValue))
		                mType->isPrice = intValue;

                    if(readXMLInteger(tmpNode, "ride", intValue))
		                mType->isRide = intValue;

                    if(readXMLInteger(tmpNode, "fly", intValue))
		                mType->isFly = intValue;

                    if(readXMLInteger(tmpNode, "surf", intValue))
		                mType->isSurf = intValue;

                    if(readXMLString(tmpNode, "dig", strValue))
		                mType->isDig = booleanString(strValue);

                    if(readXMLString(tmpNode, "rocksmash", strValue))
		                mType->isRockSmash = booleanString(strValue);

                    if(readXMLString(tmpNode, "cut", strValue))
		                mType->isCut = booleanString(strValue);

                    if(readXMLString(tmpNode, "blink", strValue))
		                mType->isBlink = booleanString(strValue);

		            if(readXMLString(tmpNode, "light", strValue))
		                mType->isLight = booleanString(strValue);

                    if(readXMLString(tmpNode, "cmid", strValue))
		                mType->isCMid = booleanString(strValue);

                    if(readXMLString(tmpNode, "headbutt", strValue))
		                mType->isHeadButt = booleanString(strValue);
				}
