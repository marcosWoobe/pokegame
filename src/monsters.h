...
{
    std::string isName;
	bool combatSpell, isMelee, automatic; 
...
class MonsterType
{
	public:
		...
		// Item* createLoot(const LootBlock& lootBlock, float lucky = 0);
		...
		bool isSummonable, isIllusionable, isConvinceable, isAttackable, isHostile, isLureable,
			isWalkable, canPushItems, canPushCreatures, pushable, hideName, hideHealth, isPassive, isShiny;
		...
		uint32_t yellChance, yellSpeedTicks, staticAttackChance, manaCost, isPrice;
		uint16_t isRide, isFly, isSurf;
        bool isRockSmash, isDig, isCut, isBlink, isLight, isCMid, isHeadButt;
		uint64_t experience;
