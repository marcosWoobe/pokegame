...
bool IOMapSerialize::loadItems(Database* db, DBResult* result, Cylinder* parent, bool depotTransfer/* = false*/)
{
	...
	do
	{
		...
		if(iType.moveable || iType.forceSerialize || pid || iType.type == ITEM_TYPE_CARPET)// || iType.alwaysOnTop && (!iType.isDoor() || !iType.isBed()))
		{
...
bool IOMapSerialize::saveItemsViktor(Database* db, uint32_t& tileId, uint32_t houseId, const Tile* tile)
{
	...
	{
		Item* item = NULL;
		if(!(item = tile->__getThing(i)->getItem()) || (item->isNotMoveable() && !item->forceHouseSerialize()))
			continue;
		...
		query << "INSERT INTO `tile_items` (`tile_id`, `world_id`, `sid`, `pid`, `itemtype`, `count`, `attributes`) VALUES (" << tileId 
		<< ", " << g_config.getNumber(ConfigManager::WORLD_ID) 
		<< ", " << ++runningId 
		<< ", " << parentId 
		<< ", "	<< item->getID() 
		...
		if(!db->executeQuery(query.str()))
		{
			std::cout << "**SAVE ITEM HOUSE DB**" << std::endl;	
			continue;
		}
...
bool IOMapSerialize::saveTile(PropWriteStream& stream, const Tile* tile)
{
	...
	for(; tileCount > 0; --tileCount)
	{
		if((item = tile->__getThing(tileCount - 1)->getItem()) &&
			(item->isMoveable() || item->forceHouseSerialize()))
