...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch(attr)
	{
		case ATTR_COUNT:
		{
			...
			//setSubType((uint16_t)_count); //65kitem
			break;
		}
...
int32_t Item::getAllPokeBalls() const
{
	return items[id].pokeBall;
}
...
{
   // Container* container = item->getContainerByID(item->getID());
	std::stringstream s;
	s << "Você está vendo ";
	const ItemType& itss = it;
	if(item){
        //itss = Item::items[item->getID()];
		if(itss.pokeBall){
			if(item->getBallOn())
				s << item->getBall() << " ball.\n";
   			else if(item->getBallUse())
           	    s <<  "using" << item->getBall() << " ball.\n";
     	    else if(item->getBallOff())
                s << "fainted" << item->getBall() << " ball.\n";
              
			s << "contains a " << item->getPoke();
            if(item->isPokesNick())
            	s << " (" << item->getPokeNick() << ")";
              
			if(item->isPokesBoost())
            	s << " + " << item->getPokeBoost();
		...
		if(it.priceItem){
			if(item->getItemCount() > 1)                       
				s << ". Price: $" << it.priceItem * item->getItemCount();
			else
				s << ". Price: $" << it.priceItem;
		}
	}else
		s << getNameDescription(it, item, subType, addArticle);
	...
	if(itss.pokeBall)
	    s << "";
	else if(item && !item->getSpecialDescription().empty())
...
boost::any value = getAttribute("poke");
if(value.type() == typeid(std::string))
    return true;

return false;
}
bool Item::isPokesNick() const{
boost::any value = getAttribute("nick");
if(value.type() == typeid(std::string))
    return true;

return false;
}
bool Item::isPokesBoost() const{
boost::any value = getAttribute("boost");
int32_t v = boost::any_cast<int32_t>(value);
if(value.type() == typeid(int32_t) && v >= 1)
    return true;

return false;
}
bool Item::getBallOn() const{
boost::any value = getAttribute("balls");
const char* v = boost::any_cast<std::string>(value).c_str();
std::string on = "on";
if(value.type() == typeid(std::string) && v == on)
 return true;

return false;
}
bool Item::getBallUse() const{
boost::any value = getAttribute("balls");
const char* v = boost::any_cast<std::string>(value).c_str();
std::string use = "use";
if(value.type() == typeid(std::string) && v == use)
 return true;

return false;
}
bool Item::getBallOff() const{
boost::any value = getAttribute("balls");
const char* v = boost::any_cast<std::string>(value).c_str();
std::string off = "off";
if(value.type() == typeid(std::string) && v == off)
 return true;

return false;
}
std::string Item::getPPrice() const{
                    boost::any value = getAttribute("poke");
	const char* v = boost::any_cast<std::string>(value).c_str();
    MonsterType* mType = g_monsters.getMonsterType(v);
	uint32_t vks = getPokeBoost() * 5000;
	std::stringstream s;
	if(mType->isPrice){
	s << ". Price: $" << mType->isPrice + vks;
	}else{
	s << ". Price: " << "unsellable";
}
	return s.str();
