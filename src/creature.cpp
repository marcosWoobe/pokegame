...
void Creature::setNick(std::string newNick){
    	SpectatorVec list;
    	g_game.getSpectators(list, this->getPosition());
        Player* player = NULL;
        Nick = newNick;
        for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
        {
           if(player = (*it)->getPlayer())
               {
                   player->sendCreatureDisappear(this, this->getTile()->getClientIndexOfThing(player, this));
                   player->sendCreatureAppear(this);
               }
        }
...
bool Creature::isInMasterRange()
{
    if (!isSummon() || !getMaster())
        return true;
    Position myPos = getPosition(), masterPos = getMaster()->getPosition();
    if (myPos.z != masterPos.z)
        return false;
    if (canSee(myPos, masterPos, 13, 8))
        return true;
...
// 	return stepDuration - (ct - lastStep);
// } MIXLORT CORREÇÃO DO DELAY DOS WILD TESTE
...
void Creature::onWalk()
{
	...
	{
	    // Outfit_t outfit = 230;
	    if(isSummon() && getDistanceBetween() <= 1)
	    {
            Item* ball;
            uint32_t slot = SLOT_FEET;
            ball = getMaster()->getPlayer()->getEquippedItem((slots_t)slot);
            boost::any pok = ball->getAttribute("poke");
            std::string poke = boost::any_cast<std::string>(pok);

            MonsterType* mType = g_monsters.getMonsterType(poke);

            boost::any adnow = ball->getAttribute("addonNow");
            int32_t addonNow = boost::any_cast<int32_t>(adnow);

	        boost::any col1 = ball->getAttribute("color1");
            int32_t color1 = boost::any_cast<int32_t>(col1);

	        boost::any col2 = ball->getAttribute("color2");
     	    int32_t color2 = boost::any_cast<int32_t>(col2);

	        boost::any col3 = ball->getAttribute("color3");
            int32_t color3 = boost::any_cast<int32_t>(col3);

	        boost::any col4 = ball->getAttribute("color4");
            int32_t color4 = boost::any_cast<int32_t>(col4);

	        Outfit_t outfit;
	        //outfit.lookType = mType->isRide;
    	    outfit.lookFeet = color1;
	        outfit.lookLegs = color2;
	        outfit.lookBody = color3;
	        outfit.lookHead = color4;
	        //std::cout << getMaster()->getCreature()->getOrderHabil() << std::endl;
	        if (getMaster()->getCreature()->getOrderHabil() == "fly")
			{
	        //    std::cout << "fidapiua" << std::endl;
	            outfit.lookType = mType->isFly;
	    	    Spell::CreateIllusion(getMaster(), outfit, -1);
	    	    getMaster()->setOrderHabil("");
	    	    getMaster()->setStorage(17000, "1");
	    	    g_game.removeCreature(this);
	        }
    	    else if(getMaster()->getCreature()->getOrderHabil() == "ride")
    	    {
    		    outfit.lookType = mType->isRide;
    		    Spell::CreateIllusion(getMaster(), outfit, -1);
	    	    getMaster()->setOrderHabil("");
	    	    getMaster()->setStorage(17001, "1");
	    	    g_game.removeCreature(this);
		    }
		    else if(getMaster()->getCreature()->getOrderHabil() == "surf")
    	    {
    	    	outfit.lookType = mType->isSurf;
    		    getMaster()->setOrderHabil("");
    		    if (getDistanceBetween() <= 1)
    		    {
			        Spell::CreateIllusion(getMaster(), outfit, -1);
	    		    getMaster()->setStorage(17002, "1");
	    		    Thing* tmp = getMaster()->getCreature();
	    		    g_game.internalTeleport(tmp, getMaster()->getOrderPos(), true);
	    		    //doTeleportThing(cid, getMaster()->getPosition());
	    		    g_game.removeCreature(this);
			    }else
				    getMaster()->setOrderHabil("surf");


	        }else if (getMaster()->getOrderHabil() == "rock smash")
			{
	    	    Tile* tile = g_game.getTile(getMaster()->getOrderPos());
	    	    Item* item = g_game.findItemOfType(tile, 1285, false, -1);
        	    g_game.transformItem(item, 3610, -1);
        	    Item* items = g_game.findItemOfType(tile, 3610, false, -1);
        	    getMaster()->setOrderHabil("");
	    	    Scheduler::getInstance().addEvent(createSchedulerTask(1000, boost::bind(&Game::transformItem, &g_game, items, 1285, -1)));
...
int32_t Creature::getDistanceBetween() const
{
const Position& pos = getPosition();
//const Position& pos = getMaster()->getPosition();
const Position& topos = getMaster()->getOrderPos();
       int32_t x = std::abs(pos.x - topos.x);
       int32_t y = std::abs(pos.y - topos.y);
       uint32_t diff = std::max(x, y);
...
void Creature::onWalk(Direction& dir)
{
	...
	uint32_t r = random_range(0, 8); /* PS From 16 to 8 */ //mixlort psoul
	if(r > 4)
		return;
	...
	// g_game.internalCreatureSay(this, SPEAK_MONSTER_SAY, "Hicks!", isGhost()); // mixlort teste desativado
}
...
bool Creature::onDeath()
{
    ...
    int32_t damageLoot = 0;
	for(CountMap::iterator it = damageMap.begin(); it != damageMap.end(); ++it)
	{
		if((tmp = g_game.getCreatureByID(it->first)))
			if(damageLoot > getMonster()->damageMap[tmp->getName()])
			{
			    continue;
			}else{
				tmp->onAttackedCreatureKilled(this);
				damageLoot = getMonster()->damageMap[tmp->getName()];
			}
	}
...
void Creature::dropCorpse(DeathList deathList)
{
	...
	switch(getRace())
	{
		...
		case RACE_BLOOD:
			splash = Item::CreateItem(ITEM_FULLSPLASH, FLUID_BLOOD); // masterx
			break;
...
BlockType_t Creature::blockHit(Creature* attacker, CombatType_t combatType, int32_t& damage,
	bool checkDefense/* = false*/, bool checkArmor/* = false*/) // mixlort teste
{
...
void Creature::addDamagePoints(Creature* attacker, int32_t damagePoints)
{
	...
	for(CountMap::iterator it = damageMap.begin(); it != damageMap.end(); ++it)
	{
		if((tmp = g_game.getCreatureByID(it->first)))
		{
			if(damageLoot > it->second.total){
			    continue;
			}else{
				lastHitCreature = tmp->getID();
				damageLoot = it->second.total;
			}
		}
	}
}
...
{
    // adicionar checagem de atributo
   /* Item* ball;
    uint32_t slot = SLOT_FEET;
    if (this->getPlayer() && this->getPlayer()->getEquippedItem((slots_t)slot)) {// tem alguma ball
       ball = this->getPlayer()->getEquippedItem((slots_t)slot);
       boost::any value = ball->getAttribute("nick");
       boost::any values = ball->getAttribute("nome");
       const char* v = boost::any_cast<std::string>(values).c_str();

       if (!value.empty() || v == "Ditto" || v == "Shiny Ditto"){
          if(!value.empty()){
          creature->nick = boost::any_cast<std::string>(value).c_str();
          creature->ditto = creature->nick;
          }else{
          creature->nick = boost::any_cast<std::string>(values).c_str();
          }
       }
          creature->lastNick = creature->nick;
    }*/
    // fim
...
// 	if (monster && !monster->isFleeing() && !monster->getMaster()) {
// 		stepDuration *= 2;
// 	}
...
int32_t Creature::getStepDuration(Direction dir) const
{
	...
	return getStepDuration();
}  // Mixlort correção do Walk selvagem
