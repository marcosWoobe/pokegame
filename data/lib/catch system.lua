...
if storage == -1 then
   return sendMsgToPlayer(cid, 27, "Voc� ainda n�o gastou nenhuma ball para capturar esse pokemon!")
end
...
table.insert(msg, "Voc� ja gastou: ")
...
for n, g, s, u, v, s2, d, magu, sora, yume, dusk, tale, moon, net, premier, tinker, fast, heavy in storage:gmatch(t) do
	...
	if #msg == 1 then
		return doPlayerSendTextMessage(cid, 27, "Voc� ainda n�o gastou nenhuma ball para catar um "..name..".")
	end
	...
	if pokeTab then
		...
		if math.random() >= pokeChance then
			...
			if name == tostring(getPlayerStorageValue(cid, catchModes.storage2)) and not hasCatched(cid) then
				...
				addEvent(doPlayerSendTextMessage, 3050, cid, 27, "Daily Catch: Voc� terminou a miss�o! Volte para pegar sua recompensa!")
			end
