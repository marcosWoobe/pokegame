...
if itemEx.itemid == 13911 then   --id do item  arvore do sudowoodo
   local item = getTileItemById(toPosition, 13911) --id do item  arvore do sudowoodo
   addEvent(back, choose(10) * 60, itemEx, toPosition)  --tempo de resp varia de 5~15min
   doRemoveItem(item.uid, 1)
   doSendMagicEffect(toPosition, 1)
   local poke = doCreateMonster("Shiny Arcanine", toPosition)
