...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if(getItemLevelDoor(item.itemid) > 0) then
		if (item.uid == 1150) then 
			if getPlayerStorageValue(cid, 65060) < 1 then
			    doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Desculpe, você não é digno, conquiste seu primeiro trono primeiro.")
			    return true
			else
			    doorEnter(cid, item, toPosition)
				doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Boa sorte, acredito em você, conquiste mais Thones.")
			    return true
			end
		end
