...
doPlayerSendCancel(cid, "Você não pode pescar enquanto surfa / voa.")
return true
end

if isInArray(waters, getTileInfo(getThingPos(cid)).itemid) then
doPlayerSendCancel(cid, "Você não pode pescar enquanto surfa nem voando acima da água.")
return true
end

if getTileInfo(getThingPos(getCreatureSummons(cid)[1] or cid)).protection then
	doPlayerSendCancel(cid, "Você não pode pescar pokémons se você ou seu pokémon estiver em uma zona de proteção.")
return true
end
...
doPlayerSendTextMessage(cid, 27, "Você já está pescando.")
return true
end
if getPlayerSex(cid) == 1 then
	if getCreatureOutfit(cid).lookType ~= 520 then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "'Você precisa de roupa de pescador para pescar '/' Você precisa da roupa 'Fisher' para pescar'")
	return false
	end
else
	if getCreatureOutfit(cid).lookType ~= 521 then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Você precisa de roupa de pescador para pescar.")
	return false
	end
	end
