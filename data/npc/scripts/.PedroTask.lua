...
function creatureSayCallback(cid, type, msg)
    ...
    local task = {
        ["Pedro"] = {{"Charmander", 180}, {"Charmeleon", 150}, {"Charizard", 100}},
    }
	
    local stoFinal = 9659451
    if msgcontains(msg, 'task') or msgcontains(msg, 'Task') then
        if isMyTaskComplete(cid, getNpcCid()) then
            ...
            doPlayerAddItem(cid, 2160, 10)  -- Dinheiro
			doPlayerAddItem(cid, 15644, 25) -- mighty Token
			doPlayerAddExp(cid, 1000000) -- Xp
            ...
            return true
		elseif getPlayerStorageValue(cid, stoFinal) ~= -1 then
		    local tFinal = string.explode(getPlayerStorageValue(cid, stoFinal), ",") or {}
            if isInArray(tFinal, getNpcName()) then
                selfSay("You already complete my task!", cid)
                talkState[talkUser] = 0
                return true
			end
        else
            selfSay("Then, is a simple task, i want to you kill {50 Charmander, 100 Charmeleon, 200 Charizard}, you accept the task?", cid)
