...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
      {id = 17357, qt = 10}, -- Linearly Guided Hypnose
	  {id = 12194, qt = 50}, -- future orb
	  {id = 12193, qt = 50}, -- Psychic spoon
	  {id = 13785, qt = 300}, -- enchanted gems
   }
   local rewards = {
      {id = 2160, qt = 3}, -- money
      {id = 11452, qt = 1}, -- Enigma Stone

   }
   local stoFinish = 93846
   ---------
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Hello my friend, can you bring to me: 10 Linearly Guided Hypnose, 50 future orb, 50 Psychic Spoon, 300 Enchanted Gem ? I will reward you!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      ...
      if getPlayerItemCount(cid, need[1].id) < need[1].qt then
         ...
         selfSay("Remember, you need to bring to me 10 Linearly Guided Hypnose, 50 future orb, 50 Psychic Spoon, 300 Enchanted Gem...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do
