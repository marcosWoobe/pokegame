...
local pokemons = {
    {name = "Shiny Gyarados", optionalLevel = 900, sex = SEX_MALE, nick = "", ball = "normal"},
    {name = "Tentacruel", optionalLevel = 1200, sex = SEX_MALE, nick = "", ball = "normal"},
    {name = "Shiny Vaporeon", optionalLevel = 900, sex = SEX_FEMALE, nick = "", ball = "normal"},
    {name = "Golduck", optionalLevel = 1200, sex = SEX_MALE, nick = "", ball = "normal"},
    {name = "Shiny Blastoise", optionalLevel = 900, sex = SEX_MALE, nick = "", ball = "normal"},
    {name = "Starmie", optionalLevel = 1200, sex = SEX_FEMALE, nick = "", ball = "normal"},
}
			...
			if not isCreature(getCreatureTarget(getThis())) then
				if #getCreatureSummons(challenger) >= 1 then
			   		if getCreatureOutfit(getCreatureSummons(challenger)[1]).lookType ~= 2 then --alterado v1.6
			        	--[[ doCreatureExecuteTalkAction(getThis(), "m1")
				    	doCreatureExecuteTalkAction(getThis(), "m2")
				    	doCreatureExecuteTalkAction(getThis(), "m3")
				    	doCreatureExecuteTalkAction(getThis(), "m4")
				    	doCreatureExecuteTalkAction(getThis(), "m5")
				    	doCreatureExecuteTalkAction(getThis(), "m6")
				    	doCreatureExecuteTalkAction(getThis(), "m7")
				    	doCreatureExecuteTalkAction(getThis(), "m8")
				    	doCreatureExecuteTalkAction(getThis(), "m9")]]
			    		--selfAttackCreature(getCreatureSummons(challenger)[1])
						---challenger_turn = challenger_turn + 1
						--afk_time = afk_time + 0.5
						--[[if getPlayerStorageValue(getCreatureSummons(challenger)[1], 990) == -1 then
							print("bosta")
							doSetMonsterGym(getCreatureSummons(getThis())[1], getCreatureSummons(challenger)[1])
							setPlayerStorageValue(getCreatureSummons(challenger)[1], 990, 1)
						end]]
