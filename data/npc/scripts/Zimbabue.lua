...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
      {id = 17330, qt = 30}, -- sticky hand
	  {id = 13783, qt = 500}, --Bug Gosme
   }
   local rewards = {
      {id = 2152, qt = 50}, -- money
      {id = 11443, qt = 1}, -- Venom Stone
   }
   local stoFinish = 93837
   ---------
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Hello my friend, can you bring to me: 30 Sticky Hand, 500 Bug Gosme ? I will reward you!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      ...
      if getPlayerItemCount(cid, need[1].id) < need[1].qt then
         ...
         selfSay("Remember, you need to bring to me 30 Sticky Hand, 500 Bug Gosme...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do
