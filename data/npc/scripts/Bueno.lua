...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
      {id = 2244, qt = 100}, -- leaves
	   {id = 12341, qt = 20}, -- blue vines
   }
   local rewards = {
      {id = 2160, qt = 3}, --money
      {id = 11441, qt = 1}, --leaf stone
   }
   local stoFinish = 93829
   ---------
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Hello my friend, can you bring to me: 100 leaves, 20 Blue Vines? I will reward you!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      ...
      if getPlayerItemCount(cid, need[1].id) < need[1].qt then
         ...
         selfSay("Remember, you need to bring to me 100 leaves, 20 Blue Vines...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do
